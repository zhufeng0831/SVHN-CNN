...
def train_digit_recognition_model(model_instance: BaseModel, model_json_file, weights_file, grayscale=True):
    """
    Loads the train images - positive and Negative
    Trains the digit recognition model
    Plots the model
    Evaluates the model
    :param model_instance: Instance of CustomModel or VGG16Model
    :param model_json_file:  File to save the model
    :param weights_file: File to save the weights for the model
    :param grayscale: Convert the input images grayscale - CustomModel - True, VGG16Model - False
    :return:
    """
    if grayscale:
        channels = 1
...
def _load_neg_dataset(path, grayscale=True):
    """
    Load negative Dataset. Extract a portion other than ROI in Format 1 Images
    :param path: train or test
    :param grayscale: Should the image be converted to grayscale
    :return: Input and output of the dataset
    """
    X = []
    Y = []
    ...
    for i in range(file['/digitStruct/name'].shape[0]):
        # name, x, y, w, h = load_datum(datum)
        name = _get_image_name(i, file)
        x, y, w, h, labels = _get_bbox_info(i, file)
...
def _load_dataset():
    """
    Returns train and test set of Format 2 dataset
    :return:
    """
...
def _load_and_preprocess(grayscale=True):
    train_x, train_y, test_x, test_y = _load_dataset()
    X_train = []
    Y_train = []
    X_test = []
    Y_test = []
